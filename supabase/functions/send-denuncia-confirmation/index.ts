...
serve(async (req) => {
  ...
  let body: { email?: string; protocolo?: string; nome?: string; fromEmail?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const { email, protocolo, nome, fromEmail } = body;

  if (!email || !protocolo) {
    return new Response(JSON.stringify({ error: "Campos obrigatórios: email e protocolo" }), {
      status: 400,
      headers: { ...corsHeaders, "content-type": "application/json" },
    });
  }
  ...
  const subject = `Confirmação de abertura de denúncia – Protocolo ${protocolo}`;
  ...
  <div class="wrapper">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>Seu protocolo é ${escapeHtml(protocolo)}.</p>
