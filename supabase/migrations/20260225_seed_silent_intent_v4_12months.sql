...
BEGIN
  ...
  FOR v_company_id IN SELECT id FROM companies LIMIT 3 LOOP
    ...
    FOR v_user_id IN SELECT id FROM user_profiles WHERE company_id = v_company_id LIMIT 5 LOOP
      ...
      -- Generate data for last 12 months (Mar 2025 - Feb 2026)
      FOR j IN 0..11 LOOP 
        v_date := date_trunc('month', now()) - (j || ' month')::interval;
        
        -- 1. Generate SILENT INTENTS (Clicked but NOT submitted)
        -- Generate 5 to 20 per user per month (more varied)
        ...
        -- Generate 2 to 8 per user per month
        FOR i IN 1..(2 + floor(random() * 6)::int) LOOP
          v_token := gen_random_uuid();
          ...
          IF v_report_exists = 0 THEN
              INSERT INTO reports (
                ...
                v_company_id,
                'Test Report ' || i,
                'Auto generated report for testing silent intent stats (12 months)',
                'Outros',
                'other',
                'individual',
                'first_time',
