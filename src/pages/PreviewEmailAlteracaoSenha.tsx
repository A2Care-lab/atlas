...
export default function PreviewEmailAlteracaoSenha() {
  const q = useQuery()
  const nome = q.get('nome') || 'Cenerli Alexandre'
  const url = q.get('url') || 'https://atlas.a2care.com.br'
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-5xl mx-auto bg-white border rounded shadow">
        <div className="px-4 py-3 border-b">
          <div className="text-sm text-gray-600">Preview do template de e-mail de alteração de senha</div>
          <div className="text-xs text-gray-500">Parâmetros: nome, url</div>
        </div>
        <div className="p-0">
          <iframe title="email-preview-reset" style={{ width:'100%', height:'80vh', border:0 }} srcDoc={html} />
        </div>
      </div>
    </div>
  )
