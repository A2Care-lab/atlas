...
export default function SettingsTabs() {
  ...
  const getActiveTab = (): 'users' | 'companies' | 'corporate' => {
    const path = location.pathname.toLowerCase();
    if (path.includes('/configuracoes/areas') || path.includes('/configurações/areas')) {
      return 'corporate';
    }
    const params = new URLSearchParams(location.search);
    const tab = (params.get('tab') as 'users' | 'companies' | 'corporate' | null) ?? 'users';
    return tab === 'companies' ? 'companies' : tab === 'corporate' ? 'corporate' : 'users';
  };
